...
async def live(ctx, args):
    ...
    cardCollection = await globalValues.catalog.getUser(id).getLive()
    cardCollection = deepcopy(cardCollection)

    if len(cardCollection) == 0:
        embedVar = discord.Embed(title="Live Cards", description="Showing cards carried by <@" + str(id) + ">\n\nThe list is empty.", color=0x6280D1)
    ...
    embedVar = discord.Embed(title="Live Cards", description="Showing cards carried by <@" + str(id) + ">", color=0x6280D1)
